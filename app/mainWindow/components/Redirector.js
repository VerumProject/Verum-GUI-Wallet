...
class Redirector extends Component<Props, State> {
  ...
  constructor(props?: Props) {
    ...
    this.goToVerminting = this.goToVerminting.bind(this);
  ...
  componentDidMount() {
    ...
    eventEmitter.on('goToVerminting', (address) => {
      this.goToVerminting(address)
    });
  ...
  componentWillUnmount() {
    ...
    eventEmitter.off('goToVerminting', this.goToVerminting);
  ...
  goToVerminting = (address) => {
    this.setState({
      verminting: true,
      vermintAddress: address
    });
  };
  ...
  render() {
    ...
    if (donate === true && !pathname.includes('/send')) {
      return (
        <Redirect to={`/send/${donatenfo.addrIess}/${donateInfo.paymentID}`} />
      );
    }

    if (verminting === true && !pathname.includes('/send')) {
      let vermintingAddress = this.state.vermintAddress;
      return (
        <Redirect to={`/send/${vermintingAddress}`} />
      );
    }
